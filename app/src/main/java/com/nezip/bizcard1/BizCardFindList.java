...
public class BizCardFindList extends Activity {

	static int curPage = Info.BizCardFindPage;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.bizcardlist_2200_list);
		...
		userList.startAnimation(new AnimationUtils().loadAnimation(
				BizCardFindList.this, R.anim.push_left_in));
		...
		//pageTitle.setText("나를 등록한 거래처");

		String cnt = getData(0);
		Info.BizCardFindTotalPage = (int) (Double.parseDouble(cnt) + (cnt.indexOf('.') > 0 ? 1
				: 0));
		if (Info.BizCardFindTotalPage < Info.BizCardFindPage) {
			Info.BizCardFindPage = 1;
		}
		pageIndex.setText(Info.BizCardFindPage + " / " + Info.BizCardFindTotalPage);
		if (Info.BizCardFindTotalPage > 1) {
		...
		if (Info.BizCardFindPage <= 1) {
			prev.setBackgroundResource(R.drawable.left);
		} else {
			prev.setBackgroundResource(R.drawable.left);
		}
		if (Info.BizCardFindPage >= Info.BizCardFindTotalPage) {
			...
			public void handleMessage(Message msg) {
				if (msg.what == 1) {
					...
					if (!firstSettingEnd) {
						...
						MyAdapter = new MyAdapter(BizCardFindList.this,
								R.layout.bizcardlistitem_2200_list, members);
					...
					if (members.isEmpty() && Info.BizCardFindTotalPage == 0) {
						Toast.makeText(BizCardFindList.this, "자료가 없습니다.", 0).show();

					} else if (Info.replay && MyAdapter.isEmpty()
							&& Info.BizCardFindPage <= Info.BizCardFindTotalPage) {
 
						if(Info.BizCardFindPage == 0)Info.BizCardFindPage = 1;	
						...
						userList.setSelection(Info.BizCardFindPosition);
					}

					Info.replay = true;
					BizCardFindList.curPage = Info.BizCardFindPage;
		...
		change_img.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					Intent intent = new Intent(getApplicationContext(), BizCardFindGrid.class);
		...
		searchBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					Intent intent = new Intent(getApplicationContext(), BizCardFindSearchText.class);
					intent.putExtra("SEARCHTEXT", "");
	...
	public class MyAdapter extends ArrayAdapter<UserData> {
		...
		public View getView(final int position, View convertView,
			...
			try {
				...
				final TextView userName01 = (TextView) convertView						.findViewById(R.id.userName01);
				...
				if (!"".equals(arSrc.get(position).MOBILE)) {
					...
					mobile.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							try {
								Info.performDial(BizCardFindList.this, arSrc.get(position).MOBILE);
							} catch (OutOfMemoryError e) {
								// System.gc();
							} catch (Exception e) {
							}
				...
				userName01.setText(arSrc.get(position).USER_NAME);
				...
				RowItem.setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						try {
							Info.BizCardFindPosition = position;
	...
	class MyAsyncTask extends AsyncTask<Void, String, Void> {
		...
		protected void onPreExecute() {
			...
			if (progressDialog != null && progressDialog.isShowing()) {
				...
				progressDialog = new ProgressDialog(BizCardFindList.this);
			}
	...
	static String getData(int page) {
		...
		String result = Info.Submit(
				"http://swcbizcard.nezip.co.kr/BizCardFindAction.asp", params);
	...
	static void loadXML(Context context) throws Exception {
		...
		sql = sql
				+ " WHERE PAGE = '"
				+ ("0000000000" + Info.BizCardFindPage)
						.substring(("0000000000" + Info.BizCardFindPage).length() - 10)
				+ "'";
		...
		String result = getData(Info.BizCardFindPage);
				...
				if (!"".equals(messageRow[i].trim())) {
					...
					if (messageCol.length > 17) {
						...
						sql = "INSERT INTO CORPMEMBER ( USER_EMAIL, USER_NAME, COMPANY_NAME, DEPT, GRADE, OFFICE_ADDR, UPMOO, OFFICE_TEL, OFFICE_FAX, MOBILE, USER_USE_DT, WORK_EMAIL, TOP_PHONE, HOME_PAGE, KEYWORD, COMPANY_LOGO, MY_PHOTO, REG_ID, ID, PAGE, POSITION) VALUES ( '"
								...
								+ "', '"
								+ ("0000000000" + Info.BizCardFindPage)
										.substring(("0000000000" + Info.BizCardFindPage)
												.length() - 10)
								+ "', '"
								+ ("0000000000" + i)
	...
	public void onResume() {
		...
		if (BizCardFindList.curPage != Info.BizCardFindPage) {
			if (Info.BizCardFindPage < Info.BizCardFindTotalPage) {
	...
	public void prevBtnClick() {
		if (isLoading)
			return;
		if (Info.BizCardFindPage > 1) {
			Info.BizCardFindPage--;
			Info.BizCardFindPosition = 0;

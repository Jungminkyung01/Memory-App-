...
public class BizCardFindGrid extends Activity {
	...
	int lastPosition = 0; // GridView 마지막 position
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		userGrid.startAnimation(new AnimationUtils().loadAnimation(
				BizCardFindGrid.this, R.anim.push_left_in));
			...
			public void handleMessage(Message msg) {
				if (msg.what == 1) {
					...
					if (!firstSettingEnd) {
						...
						MyAdapter = new MyAdapter(BizCardFindGrid.this,
					...
					if (members.isEmpty() && Info.BizCardFindTotalPage == 0) {
						Toast.makeText(BizCardFindGrid.this, "자료가 없습니다.", 0).show();
						...
						userGrid.setSelection(Info.BizCardFindPosition);
					}
				...
				if (!"".equals(arSrc.get(position).MOBILE)) {
					...
					mobile.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							...
							try {
								Info.performDial(BizCardFindGrid.this, arSrc.get(position).MOBILE);
							} catch (OutOfMemoryError e) {
								// System.gc();
							} catch (Exception e) {
							}
	...
	class MyAsyncTask extends AsyncTask<Void, String, Void> {
		...
		protected void onPreExecute() {
			...
			if (progressDialog != null && progressDialog.isShowing()) {
				...
				progressDialog = new ProgressDialog(BizCardFindGrid.this);
			}
